---
title: Running Azure Functions on Azure Kubernetes Services
date: '2024-08-26'
tags: ['Azure', 'Azure Functions', 'AKS', 'Kubernetes', 'Container']
images: []
draft: true
summary: Explore the benefits of running Azure Functions on Azure Kubernetes Services (AKS).
---
...
Microsoft provides a set of base images for the Azure Functions runtime targeting different programming languages which you can use to build your custom images. The base images can be found [here](https://mcr.microsoft.com/catalog?search=functions).
...
It is a Cloud Native Computing Foundation (CNCF) sandbox project and is installed as an add-on to the Kubernetes cluster.
More information about KEDA can be found [here](https://keda.sh/).
...
When a message or event arrives at the specified Kafka Topic, it detects this event and activates the deployment using HPA.

There are scalers available for various sources - messaging (Azure Service Bus, Kafka, RabbitMQ), Database (MySQL, PostgreSQL, CosmosDB), and others. The full list of scalers can be found [here](https://keda.sh/docs/2.15/scalers/).
